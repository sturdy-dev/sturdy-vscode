...
suite('Extension Test Suite', () => {
    ...
    test('conflict messages', async () => {
        ...
        assert.strictEqual(alert.showMessage, true)
    ...
    test('conflict messages only conflicting with PRs', async () => {
        let conflicts: Array<Conflict> = [
            {
                id: "abc123",
                repository_id: "abc123",
             ...
             {
                id: "abc444",
                repository_id: "abc444",
                base: "111111",
                onto: "222222",
                onto_name: "github-pr-444",
                onto_reference_type: "github-pr",
                onto_reference_id: "444",
                conflicting: true,
                is_conflict_in_working_directory: true,
                checked_at: "2021-02-02",
                user_id: "abc444",
        ...
        let alert = AlertMessageForConflicts(c)
        assert.strictEqual(alert.showMessage, false)
        assert.strictEqual(alert.message, "Your uncommitted changes are conflicting with #123, #444. ");

        let statusBar = StatusBarMessageForConflicts(c)
        assert.strictEqual(statusBar.msg, "$(warning) 2 PRs")
    });
